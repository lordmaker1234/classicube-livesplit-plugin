...
fn serialized_text_is_readable_and_label_free() {
    ...
    // Bare keyword lines only: the title ("any%") and segment labels
    // ("start", "lobby", "end") never appear, there's no `LS label` line,
    // and no inline label rides after the coords / map name. The title
    // lives in the `.lss` `<CategoryName>`, not the payload.
    assert_eq!(
        text,
...
fn round_trip_via_parse_defaults_start_label() {
    ...
    let text = serialize_canonical(&t).unwrap();
    let back = parse(&text, t.name.clone(), segment_labels_of(&t)).unwrap();

    // Geometry, kinds, and non-start labels round-trip exactly. The name
    // comes from the `title` arg (the `.lss` `<CategoryName>`), not the
    // payload.
...
#[test]
fn track_name_change_yields_equal_text() {
    // The track name is no longer in the canonical payload (it lives in
    // the `.lss` `<CategoryName>`), so a rename yields identical text. A
    // renamed track diverges via its filename category stem, not the
    // dedup text.
    let a = sample_track();
    let mut b = sample_track();
    b.name = "100%".to_owned();
...
fn parse_substitutes_placeholder_for_empty_label() {
    ...
    let back = parse(&text, t.name.clone(), labels).unwrap();
...
fn parse_rejects_label_count_mismatch() {
    ...
    let text = serialize_canonical(&t).unwrap();
    // Too few.
    assert!(parse(&text, t.name.clone(), vec!["only".into()]).is_err());
...
fn parse_tolerates_indentation_and_crlf() {
    ...
    let messy = text
        ...
        .collect::<Vec<_>>()
        .join("\r\n");
    let back = parse(&messy, t.name.clone(), segment_labels_of(&t)).unwrap();
    let clean = parse(&text, t.name.clone(), segment_labels_of(&t)).unwrap();
